...
import (
	...
	"github.com/newstack-cloud/bluelink/apps/cli/internal/tui/stateexportui"
...
// errStateExportFailed is a sentinel error used to indicate state export failed
// The actual error details have already been displayed by the TUI.
...
func setupStateCommand(rootCmd *cobra.Command, confProvider *config.Provider) {
	...
	setupStateExportCommand(stateCmd, confProvider)
...
func setupStateExportCommand(stateCmd *cobra.Command, confProvider *config.Provider) {
	exportCmd := &cobra.Command{
		Use:   "export",
		Short: "Export state to a file",
		Long: `Export deploy engine state to a local file or remote object storage.
...
Examples:
  ...
  bluelink state export --file ./backup/state.json

  # Export specific instances by name or ID
  bluelink state export --file ./backup/state.json --instances my-stack,inst-abc123

  # Export to S3
  bluelink state export --file s3://my-bucket/state.json

  # Export to GCS
  bluelink state export --file gcs://my-bucket/state.json

  # Export to Azure Blob Storage
  bluelink state export --file azureblob://my-container/state.json

  # Use deploy engine config to determine storage backend (flag inherited from state command)
  bluelink state --engine-config-file ~/.bluelink/engine/config.json export --file ./state.json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			cmd.SilenceUsage = true
			...
			// Parse instance filters from comma-separated string
	...
	confProvider.BindEnvVar("stateExportFile", "BLUELINK_CLI_STATE_EXPORT_FILE")
	...
	confProvider.BindEnvVar("stateExportInstances", "BLUELINK_CLI_STATE_EXPORT_INSTANCES")
	...
	confProvider.BindEnvVar("stateExportJson", "BLUELINK_CLI_STATE_EXPORT_JSON")
