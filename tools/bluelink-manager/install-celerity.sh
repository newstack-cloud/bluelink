#!/bin/sh
# Celerity bootstrap installer
#
# This script downloads and runs the bluelink-manager binary,
# which handles the full installation of Celerity components.
#
# Usage:
...
main() {
    ...
    if [ ! -f "$TMP_DIR/bluelink-manager" ]; then
        error "Installer binary not found in archive"
    fi

    mv "$TMP_DIR/bluelink-manager" "$BIN_DIR/bluelink-manager"
    chmod +x "$BIN_DIR/bluelink-manager"

    success "Downloaded installer v$VERSION"

    # Run the manager to complete installation with celerity profile
    info "Running Celerity installer..."
    exec "$BIN_DIR/bluelink-manager" install --profile celerity "$@"
}
