// Tests for the GetBlueprintInstanceExports method in the DeployEngine client.
...
func (s *ClientSuite) Test_get_blueprint_instance_exports() {
	...
	exports, err := client.GetBlueprintInstanceExports(
		context.Background(),
		testInstanceID,
	)
	s.Require().NoError(err)
...
func (s *ClientSuite) Test_get_blueprint_instance_exports_fails_for_unauthorised_client() {
	...
	_, err = client.GetBlueprintInstanceExports(
		context.Background(),
		testInstanceID,
	)
	s.Require().Error(err)
...
func (s *ClientSuite) Test_get_blueprint_instance_exports_fails_due_to_invalid_json_response() {
	// Create a new client with OAuth2.
	client, err := NewClient(
		WithClientEndpoint(s.deployEngineServer.URL),
		WithClientAuthMethod(AuthMethodOAuth2),
...
func (s *ClientSuite) Test_get_blueprint_instance_exports_fails_due_to_internal_server_error() {
	// Create a new client with OAuth2.
	client, err := NewClient(
		WithClientEndpoint(s.deployEngineServer.URL),
		WithClientAuthMethod(AuthMethodOAuth2),
...
func (s *ClientSuite) Test_get_blueprint_instance_exports_fails_due_to_network_error() {
	// Create a new client with OAuth2.
	client, err := NewClient(
		WithClientEndpoint(s.deployEngineServer.URL),
		WithClientAuthMethod(AuthMethodOAuth2),
